...
func (rep *FolderRepository) Index(r *requests.FolderIndexReqStruct) ([]model.FolderModel, error) {
	...
	query := fmt.Sprintf("SELECT * FROM folders where id IN (SELECT saved_folders.folder_id FROM saved_folders INNER JOIN folders ON %s order by saved_folders.created_at %s) and name like :query limit :limit offset :offset", filterQuery, saveOrder)

	searchStringCount := fmt.Sprintf("FROM folders where id IN (SELECT saved_folders.folder_id FROM saved_folders INNER JOIN folders ON %s order by saved_folders.created_at %s) and name like :query", filterQuery, saveOrder)
